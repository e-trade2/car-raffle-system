...
// ---- Admin-taken numbers currently held for a raffle ----
// Powers the "Release Tickets" panel on the raffle card: before the admin
// types numbers to release, show them what's actually admin-taken right
// now (as opposed to sold-to-a-real-customer, which this deliberately
// excludes - releasing a real sale needs Unconfirm -> Reject, not this).
router.get('/raffles/:id/admin-taken', (req, res) => {
  const data = db.load();
  const raffle = data.raffles.find(r => r.id === req.params.id);
  if (!raffle) return res.status(404).json({ error: 'Raffle not found' });

  const numbers = data.orders
    .filter(o => o.raffleId === raffle.id && o.adminTaken && o.status === 'confirmed')
    .flatMap(o => o.ticketNumbers)
    .sort((a, b) => a - b);
  res.json({ numbers, count: numbers.length });
});

// ---- Release admin-taken tickets for a raffle, without picking an order ----
// Same underlying action as POST /orders/:id/admin-release below, but keyed
// by raffle instead of by order id - a single admin-take can quietly span
// several orders (e.g. one earlier batch plus a later top-up), and asking
// the admin to go find each order id in the Orders tab just to release a
// few numbers is unnecessary friction when they're already looking at the
// raffle card. Sweeps every confirmed, adminTaken order for this raffle and
// releases the requested numbers out of each. Omit `numbers` to release
// every admin-taken number on this raffle in one go.
router.post('/raffles/:id/admin-release', (req, res) => {
  let data = db.load();
  const raffle = data.raffles.find(r => r.id === req.params.id);
  if (!raffle) return res.status(404).json({ error: 'Raffle not found' });

  const adminOrders = data.orders.filter(o => o.raffleId === raffle.id && o.adminTaken && o.status === 'confirmed');
  ...
  if (Array.isArray(numbers) && numbers.length) {
    const normalized = numbers.map(n => Number.parseInt(n, 10));
    if (normalized.some(n => !Number.isInteger(n))) {
  ...
  for (const order of adminOrders) {
    const toRelease = targetNumbers
      ? order.ticketNumbers.filter(n => targetNumbers.has(n))
      : order.ticketNumbers.slice();
    if (!toRelease.length) continue;
    ...
    if (remaining.length) {
      // Partial release - order stays 'confirmed', just smaller now.
  ...
  db.save(data);
  res.json({ released, raffle: publicRaffle(raffle) });
});
