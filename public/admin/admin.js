    ...
    <div class="card" style="padding:14px;margin-bottom:10px;">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px;">
        <div>
          ...
          ${a.imageUrl ? `<img src="${esc(a.imageUrl)}" style="max-width:160px;border-radius:8px;display:block;margin-top:6px;">` : ''}
          ${a.winner ? `
...
document.getElementById('annImageFile').addEventListener('change', (e) => {
  const file = e.target.files[0];
  const wrap = document.getElementById('annImagePreviewWrap');
  if (!file){ wrap.style.display = 'none'; return; }
  document.getElementById('annImagePreview').src = URL.createObjectURL(file);
...
document.getElementById('postAnnBtn').addEventListener('click', async () => {
  ...
  body.imageUrl = document.getElementById('annImageUrl').value.trim();

  const fileInput = document.getElementById('annImageFile');
  const postBtn = document.getElementById('postAnnBtn');
  postBtn.disabled = true;
  try {
    // Same optional-photo pattern as raffle creation: a picked file wins
    // over anything typed into the URL field; if neither is set the
    // announcement just posts with no image.
    if (fileInput.files && fileInput.files[0]){
      const fd = new FormData();
      fd.append('photo', fileInput.files[0]);
      const uploaded = await apiForm('/announcements/photo', fd);
    ...
    fileInput.value = '';
    document.getElementById('annImagePreviewWrap').style.display = 'none';
